...
def _dispatch_request(*_args, authorized_request=True):
    ...
    def outer_wrapper(f):
        ...
        def sync_wrapper(self, *args, **kwargs):
            ...
            request_factory = getattr(super(self.__class__, self), ('_prep_' + f.__name__))
        ...
        @wraps(f)
        async def async_wrapper(self, *args, **kwargs):
            args_with_injections, kwargs_with_injections = await f(self, *args, **kwargs)
            request_factory = getattr(super(self.__class__, self), ('_prep_' + f.__name__))
            request = request_factory(*args_with_injections, **kwargs_with_injections)
