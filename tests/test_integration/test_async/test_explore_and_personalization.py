...
async def test_recommendations(async_spotify_user_auth):
    assert await async_spotify_user_auth.recommendations(market='US', seed_tracks='0c6xIDDpzE81m2q797ordA', min_energy=0.4, min_popularity=50)
...
async def test_tracks_audio_features(async_spotify_user_auth, pound_cake_track_id, gods_plan_track_id):
    await async_spotify_user_auth.tracks_audio_features([pound_cake_track_id, gods_plan_track_id])
